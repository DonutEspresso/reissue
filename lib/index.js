 ...
 * @param {Boolean} [opts.immediate] when true, start queue immediately
 */
function Reissue(opts) {
    ...
    assert.optionalBool(opts.immediate, 'immediate');
    ...
    /**
     * when true, immediately kick off first invocation upon completion of
     * constructor.
     * @type {Boolean}
     */
    self._immediate = opts.immediate || false;
    ...
    // kick off the timer if immediate is true
    if (self._immediate === true) {
        self.start();
    }
}
 ...
 * @return {undefined}
 */
Reissue.prototype.start = function start() {
    var self = this;
